...
impl Controller {
    ...
    fn write_output_network(&mut self, output_path: PathBuf) {
        let net_file_name = if let Some(path) = &self.scenario.config.network().path {
            io::resolve_path(self.scenario.config.context(), path)
        } else {
            // to be adapted
            PathBuf::from("output_network.xml.gz")
        };

        let net_in_path = io::resolve_path(self.scenario.config.context(), &net_file_name);
