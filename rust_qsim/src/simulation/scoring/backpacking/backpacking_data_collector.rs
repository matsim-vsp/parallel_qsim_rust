...
impl BackpackingDataCollector {
    ...
    /// Drains the scoring message channel into this collector's authoritative maps, blocking on
    /// each pending wait_for_backpack/wait_for_vehicle registration until it is satisfied. Splits
    /// self into disjoint field borrows so the broker can write directly into the maps and clear
