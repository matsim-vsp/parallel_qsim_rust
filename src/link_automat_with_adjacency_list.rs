...
pub fn run() {
    ...
    let link1 = Link { q: VecDeque::new(), buffer: VecDeque::from([vehicle1]), id: 1 };
    let link2 = Link { q: VecDeque::new(), buffer: VecDeque::from([vehicle2]), id: 2 };
    let link3 = Link { q: VecDeque::new(), buffer: VecDeque::new(), id: 3 };
    let mut links = vec![link1, link2, link3];

    let node1 = Node { id: 1, in_links: Vec::new(), out_links: Vec::from([0]) };
    let node2 = Node { id: 2, in_links: Vec::new(), out_links: Vec::from([1]) };
    let node3 = Node { id: 3, in_links: Vec::from([0, 1]), out_links: Vec::from([2]) };
    let node4 = Node { id: 4, in_links: Vec::from([2]), out_links: Vec::new() };
...
fn move_links(links: &mut Vec<Link>) {
    ...
    for link in links {
        match link.q.pop_front() {
            ...
            Some(vehicle) => {
                println!("Pushing vehicle #{} on link #{} from q to buffer", vehicle.id, link.id);
